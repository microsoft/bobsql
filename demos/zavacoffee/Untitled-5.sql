...
------------------------------------------------------------
-- Creates one Shipment per (supplier, store) grouping of APPROVED POs for @asof_date.
-- Links POs to the Shipment and flips them to DISPATCHED.
------------------------------------------------------------
CREATE OR ALTER PROCEDURE zava.sp_CreateShipmentsFromApprovedPOs
  @asof_date DATE = NULL
AS
BEGIN
  SET NOCOUNT ON;
  ...
  ;WITH ReadyPO AS (
    SELECT po.po_id, po.supplier_id, po.store_id,
           MIN(po.expected_delivery_date) AS expected_date
    FROM zava.PurchaseOrder po
    WHERE po.status = 'APPROVED'
      AND po.order_date = @asof_date
    GROUP BY po.po_id, po.supplier_id, po.store_id
  ),
  ShipGroups AS (
    SELECT supplier_id, store_id,
           MIN(expected_date) AS min_expected
    FROM ReadyPO
    GROUP BY supplier_id, store_id
  )
  -- Create shipment headers
  INSERT INTO zava.Shipment (shipment_number, supplier_id, store_id, status, planned_pickup_utc, planned_delivery_utc, incoterms)
  SELECT
    CONCAT('SH-', NEXT VALUE FOR zava.seq_shipment_number),
    g.supplier_id,
    g.store_id,
    'DISPATCHED',
    DATEADD(HOUR, 0, CAST(@asof_date AS DATETIME2(0))),             -- assume same-day pickup
    DATEADD(DAY, 0, CAST(g.min_expected AS DATETIME2(0))),           -- plan by earliest expected
    s.incoterms
  FROM ShipGroups g
  JOIN zava.Supplier s ON s.supplier_id = g.supplier_id;

  -- Link POs to shipments
  ;WITH MapPO AS (
    SELECT sh.shipment_id, sh.supplier_id, sh.store_id
    FROM zava.Shipment sh
    WHERE sh.status = 'DISPATCHED'
      AND CAST(sh.created_utc AS DATE) = CAST(SYSUTCDATETIME() AS DATE)
  )
  INSERT INTO zava.ShipmentPO (shipment_id, po_id)
  SELECT m.shipment_id, rp.po_id
  FROM MapPO m
  JOIN ReadyPO rp
    ON rp.supplier_id = m.supplier_id AND rp.store_id = m.store_id;

  -- Update POs to DISPATCHED
  UPDATE po
  SET po.status = 'DISPATCHED',
      po.dispatched_utc = SYSUTCDATETIME()
  FROM zava.PurchaseOrder po
  WHERE po.po_id IN (SELECT po_id FROM zava.ShipmentPO);
END
GO
